(function () {
  ...
  function renderAtividadesApoio() {
    ...
    function refreshApoioLista() {
      ...
      lista.innerHTML = state.atividades.map((a, i) => `
        <div class="border rounded p-2 mb-2">
          ...
          Local: ${escapeHtml(a.local)} | Horário: ${escapeHtml(a.horario_inicio || a.horario || '')} até ${escapeHtml(a.horario_termino || '')} | Portaria: ${escapeHtml(a.portaria)} | CH semanal: ${a.ch_semanal}h
    ...
    document.getElementById('ativ-apoio-add').addEventListener('click', () => {
      ...
      document.querySelectorAll('input[name="apoio-cb"]:checked').forEach(cb => {
          ...
          <div class="row g-2 mb-2">
            ...
            <div class="col-md-3">
              ...
              <div class="d-flex align-items-center gap-1">
                <input type="time" class="form-control form-control-sm apoio-inp-horario-inicio">
                <span class="small text-muted">até</span>
                <input type="time" class="form-control form-control-sm apoio-inp-horario-termino">
              </div>
            </div>
        ...
        const horarioInicioInp = card.querySelector('.apoio-inp-horario-inicio');
        const horarioTerminoInp = card.querySelector('.apoio-inp-horario-termino');
        ...
        card.querySelector('.apoio-confirm').addEventListener('click', () => {
          ...
          state.atividades.push({
            ...
            horario_inicio: horarioInicioInp.value || '',
            horario_termino: horarioTerminoInp.value || '',
  ...
  async function gerarPDF() {
    ...
    // Cores
    const headerBg = [220, 230, 241];  // azul claro
    const headerTxt = [0, 0, 0];
    const totalBg = [189, 215, 238];

    let y = margin;

    // ── Cabeçalho com logo ─────────────────────────────────────────
    // Carregar logo como base64 e inserir ao lado do título
    const logoH = 22;   // altura da imagem em mm
    const logoW = 16;   // largura proporcional
    const tituloX = margin + logoW + 5;

    function desenharCabecalho(imgData) {
      if (imgData) {
        doc.addImage(imgData, 'PNG', margin, y, logoW, logoH);
      }
      doc.setFontSize(13);
      doc.setFont('helvetica', 'bold');
      doc.text('PLANO INDIVIDUAL DE TRABALHO', tituloX + (contentW - logoW - 5) / 2, y + 8, { align: 'center' });
      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      doc.text('SEMESTRE: ' + (state.semestre || '___') + '/' + (state.ano || '______'), tituloX + (contentW - logoW - 5) / 2, y + 16, { align: 'center' });
    }

    // Tentar carregar a logo via fetch para base64
    let logoCarregada = false;
    try {
      const resp = await fetch('img/logo-ifpe.jpeg');
      if (resp.ok) {
        ...
        desenharCabecalho(imgData);
        logoCarregada = true;
      }
    } catch (e) { /* ignora, desenha sem logo */ }

    if (!logoCarregada) desenharCabecalho(null);

    y += logoH + 3;

    // ── Tabela: Campus / Coordenação / Docente / Grupo / Regime ────
    doc.autoTable({
      startY: y,
      ...
      body: [
        [
          { content: 'CAMPUS: ' + (d.campus || ''), styles: { cellWidth: contentW * 0.45 } },
          { content: 'COORDENAÇÃO:', styles: { cellWidth: contentW * 0.55 } }
        ],
        [
          { content: 'PROFESSOR(A): ' + (d.nome || '') + '\nSIAPE: ' + (d.siape || '') + '    E-mail: ' + (d.email || ''), colSpan: 1 },
          {
            content: 'GRUPO: ' + (d.grupo_trabalho || '') + '\nREGIME DE TRABALHO:\n' +
              (d.regime_trabalho === '20h' ? '[X] 20h  [ ] 40h  [ ] DE' :
               d.regime_trabalho === '40h' ? '[ ] 20h  [X] 40h  [ ] DE' :
               '[ ] 20h  [ ] 40h  [X] DE'),
            colSpan: 1
          }
        ]
    ...
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('AS CARGAS HORÁRIAS DEVERÃO SER INFORMADAS EM HORAS DE 60 MINUTOS', pageW / 2, y + 3, { align: 'center' });
    y += 7;

    // ── Tabela: AULAS ──────────────────────────────────────────────
    // Montar linhas por componente curricular (disciplina x turma/turno)
    const aulaRows = [];
    state.componentes_curriculares.forEach(comp => {
      const disc = state.disciplinas.find(d => d.nome === comp.disciplina);
      aulaRows.push([
        comp.disciplina + '\n(' + comp.turma + ' — ' + comp.turno + ')',
        disc ? String(disc.carga_horaria_total) + 'h' : '',
        disc ? String(disc.carga_horaria_semanal) + 'h' : '',
        disc ? String(disc.carga_horaria_preparacao) + 'h' : ''
      ]);
    });
    // linhas em branco para completar até 6 linhas mínimo
    while (aulaRows.length < 6) aulaRows.push(['', '', '', '']);
    aulaRows.push([
      { content: 'TOTAL', styles: { fontStyle: 'bold', fillColor: totalBg } },
      { content: '', styles: { fillColor: totalBg } },
      { content: dist.aulas + 'h', styles: { fontStyle: 'bold', fillColor: totalBg } },
      { content: dist.preparacao_aulas + 'h', styles: { fontStyle: 'bold', fillColor: totalBg } }
    ]);
    ...
    doc.autoTable({
      ...
      headStyles: { fillColor: headerBg, textColor: headerTxt, fontStyle: 'bold' },
      head: [[
        { content: 'AULAS', colSpan: 4, styles: { halign: 'center' } }
      ], [
        'COMPONENTES CURRICULARES', 'C.H. TOTAL DO COMPONENTE', 'C.H. SEMANAL', 'C.H. DE PREPARAÇÃO DE AULAS'
      ]],
      body: aulaRows,
      columnStyles: {
        0: { cellWidth: contentW * 0.40 },
        1: { cellWidth: contentW * 0.20, halign: 'center' },
        2: { cellWidth: contentW * 0.20, halign: 'center' },
        3: { cellWidth: contentW * 0.20, halign: 'center' }
      },
      theme: 'grid'
    });
    y = doc.lastAutoTable.finalY + 4;

    // ── Tabela: ATIVIDADES DE APOIO AO ENSINO ─────────────────────
    const apoioRows = state.atividades.map(a => {
      const hi = a.horario_inicio || a.horario || '';
      const hf = a.horario_termino || '';
      const horStr = hi ? (hf ? hi + ' até ' + hf : hi) : '';
      return [
        a.atividade,
        'Local: ' + (a.local || '') + (horStr ? '\nHorário: ' + horStr : '') + (a.portaria ? '\nPortaria: ' + a.portaria : ''),
        a.ch_semanal + 'h'
      ];
    });
    while (apoioRows.length < 5) apoioRows.push(['', '', '']);
    ...
    doc.autoTable({
      ...
      headStyles: { fillColor: headerBg, textColor: headerTxt, fontStyle: 'bold' },
      head: [[
        { content: 'ATIVIDADES DE APOIO AO ENSINO', colSpan: 3, styles: { halign: 'center' } }
      ], [
        'ATIVIDADE', 'Local / Horário / Portaria', 'C.H. Semanal'
      ]],
      body: apoioRows,
      columnStyles: {
    ...
    // ── Tabela: ATIVIDADES DE PESQUISA ────────────────────────────
    const pesqRows = state.atividades_pesquisa_pos.map(a => [a.atividade, a.carga_horaria + 'h']);
    while (pesqRows.length < 5) pesqRows.push(['', '']);
    ...
    doc.autoTable({
      ...
      headStyles: { fillColor: headerBg, textColor: headerTxt, fontStyle: 'bold' },
      head: [[
        { content: 'ATIVIDADES DE PESQUISA', colSpan: 2, styles: { halign: 'center' } }
      ], [
        'Descrição da atividade', 'C.H. Semanal'
      ]],
      body: pesqRows,
      columnStyles: {
        0: { cellWidth: contentW * 0.85 },
    ...
    // ── Tabela: ATIVIDADES DE EXTENSÃO ────────────────────────────
    const extRows = state.atividades_extensao.map(a => [
      a.atividade,
      a.projeto || '',
      a.tipo_participacao || '',
      a.inicio_ma || '',
    ...
    while (extRows.length < 4) extRows.push(['', '', '', '', '', '']);
    ...
    doc.autoTable({
      ...
      headStyles: { fillColor: headerBg, textColor: headerTxt, fontStyle: 'bold' },
      head: [[
        { content: 'ATIVIDADES DE EXTENSÃO', colSpan: 6, styles: { halign: 'center' } }
      ], [
        'Atividade', 'Projeto', 'Tipo de Participação', 'Início (M/A)', 'Término (M/A)', 'C.H. Semanal'
      ]],
      body: extRows,
      columnStyles: {
        0: { cellWidth: contentW * 0.30 },
        ...
        5: { cellWidth: contentW * 0.10, halign: 'center' }
      },
      theme: 'grid'
    });
    y = doc.lastAutoTable.finalY + 4;

    // ── Tabela: ATIVIDADES ADMINISTRATIVO-PEDAGÓGICAS ─────────────
    const adminRows = state.atividades_admin_pedagogicas.map(a => [
      a.atividade,
      a.portaria || '',
      a.inicio_ma || '',
      a.termino_ma || '',
    ...
    while (adminRows.length < 5) adminRows.push(['', '', '', '', '']);
    adminRows.push([
      { content: 'TOTAL', styles: { fontStyle: 'bold', fillColor: totalBg } },
      { content: '', styles: { fillColor: totalBg } },
      { content: '', styles: { fillColor: totalBg } },
      { content: '', styles: { fillColor: totalBg } },
      { content: dist.admin_pedagogico + 'h', styles: { fontStyle: 'bold', fillColor: totalBg } }
    ]);
    ...
    doc.autoTable({
      ...
      headStyles: { fillColor: headerBg, textColor: headerTxt, fontStyle: 'bold' },
      head: [[
        { content: 'ATIVIDADES ADMINISTRATIVO-PEDAGÓGICAS', colSpan: 5, styles: { halign: 'center' } }
      ], [
        'Atividade', 'Portaria', 'Início (M/A)', 'Término (M/A)', 'C.H. Semanal'
      ]],
      body: adminRows,
      columnStyles: {
        0: { cellWidth: contentW * 0.42 },
        1: { cellWidth: contentW * 0.20 },
        2: { cellWidth: contentW * 0.12, halign: 'center' },
        3: { cellWidth: contentW * 0.12, halign: 'center' },
    ...
    // ── Tabela: DISTRIBUIÇÃO DA CARGA HORÁRIA ─────────────────────
    doc.autoTable({
      startY: y,
      margin: { left: margin, right: margin },
      tableWidth: contentW,
      ...
      headStyles: { fillColor: headerBg, textColor: headerTxt, fontStyle: 'bold' },
      head: [[
        { content: 'DISTRIBUIÇÃO DA CARGA HORÁRIA', colSpan: 7, styles: { halign: 'center' } }
      ], [
        'AULAS', 'PREPARAÇÃO DE AULAS', 'APOIO AO ENSINO', 'PESQUISA', 'EXTENSÃO', 'ADMINISTRATIVO PEDAGÓGICO', 'TOTAL/SOMA'
      ]],
      body: [[
        { content: dist.aulas + 'h', styles: { fontStyle: 'bold' } },
        { content: dist.preparacao_aulas + 'h', styles: { fontStyle: 'bold' } },
        { content: dist.apoio_ensino + 'h', styles: { fontStyle: 'bold' } },
        { content: dist.pesquisa_pos + 'h', styles: { fontStyle: 'bold' } },
        { content: dist.extensao + 'h', styles: { fontStyle: 'bold' } },
        { content: dist.admin_pedagogico + 'h', styles: { fontStyle: 'bold' } },
        { content: dist.total + 'h', styles: { fontStyle: 'bold', fillColor: totalBg } }
      ]],
      theme: 'grid'
    });
    y = doc.lastAutoTable.finalY + 4;

    // ── Tabela: COMPLEMENTO/OBSERVAÇÕES ───────────────────────────
    const comp = state.complemento_observacoes || '';
    doc.autoTable({
      startY: y,
      margin: { left: margin, right: margin },
      tableWidth: contentW,
      ...
      headStyles: { fillColor: headerBg, textColor: headerTxt, fontStyle: 'bold' },
      head: [[{ content: 'COMPLEMENTO/OBSERVAÇÕES', styles: { halign: 'center' } }]],
      body: [[comp || ' ']],
      theme: 'grid'
    });
    ...
    if (y + 88 > doc.internal.pageSize.getHeight() - margin) {
      doc.addPage();
      y = margin;
    }
    ...
    doc.autoTable({
      ...
      margin: { left: margin, right: margin },
      tableWidth: contentW,
      styles: { fontSize: 8, cellPadding: { top: 2, bottom: 2, left: 3, right: 3 }, minCellHeight: 40, valign: 'bottom' },
      body: [[
        { content: 'DOCENTE', styles: { halign: 'center', fontStyle: 'bold' } },
        { content: 'COORDENAÇÃO', styles: { halign: 'center', fontStyle: 'bold' } },
    ...
    doc.autoTable({
      ...
      styles: { fontSize: 8, cellPadding: { top: 2, bottom: 2, left: 3, right: 3 }, minCellHeight: 40, valign: 'bottom' },
      body: [[
        { content: 'DIREÇÃO DE PESQUISA', styles: { halign: 'center', fontStyle: 'bold' } },
        { content: 'DIREÇÃO DE EXTENSÃO', styles: { halign: 'center', fontStyle: 'bold' } },
        { content: 'DIREÇÃO DE ENSINO', styles: { halign: 'center', fontStyle: 'bold' } }
      ]],
      theme: 'grid'
    });

    doc.save('PTD_' + (state.ano || 'rascunho') + '_' + (state.semestre || '') + '.pdf');
    showMsg('PDF gerado com sucesso!', 'success');
  }

  document.getElementById('btn-finalizar') && document.getElementById('btn-finalizar').addEventListener('click', () => {
    state.complemento_observacoes = document.getElementById('input-complemento').value.trim();
    downloadJSON();
    showMsg('Arquivo JSON baixado. Nome: ptd_' + state.ano + '_' + state.semestre + '.json', 'success');
  });

  document.getElementById('btn-baixar-txt') && document.getElementById('btn-baixar-txt').addEventListener('click', () => {
    downloadTXT();
